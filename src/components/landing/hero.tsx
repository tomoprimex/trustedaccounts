...
export function Hero() {
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-slate-50 to-blue-50 pt-24 lg:pt-36">
      <div className="absolute inset-0 bg-[linear-gradient(rgba(30,101,243,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(30,101,243,0.03)_1px,transparent_1px)] bg-[size:72px_72px]" />
      <div className="absolute -right-40 top-16 h-[520px] w-[520px] rounded-full bg-blue-600/20 blur-3xl" />
      <div className="absolute -left-48 bottom-0 h-[420px] w-[420px] rounded-full bg-blue-600/20 blur-3xl" />
      <div className="relative mx-auto max-w-7xl px-5 pb-20 lg:px-8 lg:pb-28">
        <div className="grid items-center gap-14 lg:grid-cols-[.92fr_1.08fr]">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65 }}>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-white/70 px-3 py-2 text-[10px] font-bold uppercase tracking-[.12em] text-blue-600 shadow-sm backdrop-blur">
              <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-blue-600" />
              The verified account marketplace
            </div>
            <h1 className="max-w-2xl text-[clamp(3rem,6vw,5.7rem)] font-extrabold leading-[.95] tracking-[-.085em] bg-gradient-to-r from-blue-600 to-blue-600 bg-clip-text text-transparent">
              The right accounts and tools<br />
              <span className="font-serif font-semibold italic text-blue-600">when you need it.</span>
            </h1>
            ...
            <div className="mt-8 flex flex-col items-start gap-4 sm:flex-row sm:items-center">
              <Link href="/signup" className="group inline-flex items-center gap-8 rounded-xl bg-gradient-to-r from-blue-600 to-blue-600 px-5 py-4 text-sm font-extrabold text-white shadow-xl shadow-blue-600/20 transition hover:-translate-y-1 hover:from-blue-500 hover:to-blue-500">
                Browse available accounts
                <ArrowRight size={17} className="transition-transform group-hover:translate-x-1" />
              </Link>
              <a href="#how-it-works" className="inline-flex items-center gap-2 text-sm font-bold text-blue-600">
                How it works
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-x-7 gap-y-3 px-5 py-5 lg:px-8">
          ...
          {platforms.map((platform, index) => (
            <span key={platform} className="inline-flex items-center gap-2 text-xs font-extrabold text-slate-500">
              <i className="text-base not-italic text-blue-600">{["◎", "▶", "♪", "f", "𝕏", "in"][index]}</i>
              {platform}
            </span>
          ))}
        </div>
...
function LoginForm({ compact = false }: { compact?: boolean }) {
  ...
  return (
    ...
    >
      <div className="absolute -inset-5 rounded-[2rem] bg-blue-600/25 blur-2xl" />
      <div className={`relative rounded-[1.5rem] border border-white/90 bg-white/70 shadow-[0_30px_80px_rgba(30,101,243,.18)] backdrop-blur-2xl ${!compact ? 'p-4 sm:p-6' : 'p-3 sm:p-4'}`}>
        <div className={`border-b border-slate-200/80 ${!compact ? 'pb-5' : 'pb-3'}`}>
          <p className="text-[10px] font-bold uppercase tracking-[.16em] text-blue-600">Sign in</p>
        ...
        <form onSubmit={handleLogin} className={`space-y-4 ${!compact ? 'mt-5' : 'mt-3'}`}>
          ...
          <div>
            ...
            <div className={`mt-1.5 relative ${!compact ? '' : 'mt-1'}`}>
              ...
              <input
                ...
                required
                className={`w-full rounded-xl border border-slate-200 bg-white/75 text-xs font-medium text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 ${!compact ? 'pl-9 pr-3 py-2.5' : 'pl-8 pr-2.5 py-2'}`}
              />
            </div>
          </div>
          <div>
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full rounded-xl bg-gradient-to-r from-blue-600 to-blue-600 text-xs font-extrabold text-white shadow-lg shadow-blue-600/20 transition hover:from-blue-500 hover:to-blue-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${!compact ? 'mt-2 px-4 py-3' : 'mt-1.5 px-3 py-2.5'}`}
          >
            {loading ? (
              <>
